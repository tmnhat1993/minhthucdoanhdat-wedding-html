...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__TimelineMax__ = __webpack_require__(4);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__TweenMax__ = __webpack_require__(16);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__CSSPlugin__ = __webpack_require__(5);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__AttrPlugin__ = __webpack_require__(6);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__RoundPropsPlugin__ = __webpack_require__(7);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7__DirectionalRotationPlugin__ = __webpack_require__(8);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8__BezierPlugin__ = __webpack_require__(9);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_9__EasePack__ = __webpack_require__(10);
...
exports.CreateInfiniteAnimation = void 0;
...
var CreateInfiniteAnimation = function CreateInfiniteAnimation($el) {
  ...
  var dur = data.TIME_MIN + Math.random() * (data.TIME_MAX - data.TIME_MIN);
  TweenMax.to($el, dur, {
    x: data.X_RANGE * Math.random() * (Math.random() < 0.5 ? -1 : 1),
    y: data.Y_RANGE * Math.random() * (Math.random() < 0.5 ? -1 : 1),
    rotation: data.DEG_RANGE * Math.random() * (Math.random() < 0.5 ? -1 : 1),
    ease: _gsap.Linear.easeNone,
    onComplete: function onComplete() {
      return CreateInfiniteAnimation($el);
    }
  });
};

exports.CreateInfiniteAnimation = CreateInfiniteAnimation;
...
var _lazyLoad = _interopRequireDefault(__webpack_require__(23));
...
var _scrollOut = _interopRequireDefault(__webpack_require__(13));

var _gsap = __webpack_require__(1);

var _utils = __webpack_require__(3);

var _Common = _interopRequireDefault(__webpack_require__(18));

var _Header = _interopRequireDefault(__webpack_require__(19));

var _Banner = _interopRequireDefault(__webpack_require__(20));

var _Gallery = _interopRequireDefault(__webpack_require__(21));

var _Sale = _interopRequireDefault(__webpack_require__(22));
...
function () {
  ...
  function Home() {
    ...
    // Page Listener
    window.SAPPOROPageListener = new _utils.pageListener();
    window.currentGameAvailable = 1; // TweenMax

    window.TweenMax = _gsap.TweenMax;
    window.TimelineMax = _gsap.TimelineMax; // Ipad devices and below
    ...
    var header = new _Header.default(); // Home page banner
    ...
    if ($("#home-sale-info").length > 0) {
      var sale = new _Sale.default();
    } // Gallery
    ...
    value: function ScrollOutSetup() {
      (0, _scrollOut.default)({
        onShown: function onShown(el) {
          ...
          SAPPOROPageListener.emit("".concat(elementID, "-anim"));
        },
        threshold: 0.225
      });
    }
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0__TweenLite__ = __webpack_require__(0);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__TweenMaxBase__ = __webpack_require__(17);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__CSSPlugin__ = __webpack_require__(5);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__AttrPlugin__ = __webpack_require__(6);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__RoundPropsPlugin__ = __webpack_require__(7);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__DirectionalRotationPlugin__ = __webpack_require__(8);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_6__TimelineLite__ = __webpack_require__(2);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7__TimelineMax__ = __webpack_require__(4);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8__BezierPlugin__ = __webpack_require__(9);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_9__EasePack__ = __webpack_require__(10);
...
function () {
  ...
  _createClass(Common, [{
    ...
    value: function bindEvents() {
      ...
      this.SmoothScrollSetup(); // Header Indicator Setup

      this.IndicatorHeaderSetup(); // Wheat Decor Animation

      this.SetupWheatDecorAnimation();
    }
    /* ===================================
     *  METHODS
     * =================================== */
    ...
    key: "IndicatorHeaderSetup",
    value: function IndicatorHeaderSetup() {
      this.$HeaderIndicator = $('.page-indicator');
      var indicateList = [];
      this.$HeaderIndicator.each(function (i, $el) {
        indicateList.push($($el).data('indicate'));
      });

      if (indicateList.length > 0) {
        indicateList.forEach(function (name) {
          SAPPOROPageListener.on("".concat(name, "-anim"), function () {
            $(".page-indicator.active").removeClass('active');
            $(".page-indicator[data-indicate=".concat(name, "]")).addClass('active');
          });
        });
      } // Exceptional Case ( Banner does not have indicator )


      SAPPOROPageListener.on('home-banner-anim', function () {
        $(".page-indicator.active").removeClass('active');
      });
    }
  }, {
    key: "SetupWheatDecorAnimation",
    value: function SetupWheatDecorAnimation() {
      this.$WheatHolder = $('.rice-decor');
      this.$Rice1Img = this.$WheatHolder.find('.rice-item.rice-1');
      this.$Rice2Img = this.$WheatHolder.find('.rice-item.rice-2');
      this.$Rice3Img = this.$WheatHolder.find('.rice-item.rice-3');
      this.decorAnimation = new TimelineMax({
        repeat: -1,
        yoyo: true,
        repeatDelay: 0.35
      });
      this.decorAnimation.add("start");
      this.decorAnimation.to(this.$Rice1Img, 4, {
        skewX: 5,
        rotation: 1,
        x: -26,
        scaleX: 0.98,
      ...
      this.decorAnimation.to(this.$Rice2Img, 3.75, {
        skewX: 7,
        rotation: 2,
        x: -30,
        scaleX: 0.98,
      ...
      this.decorAnimation.to(this.$Rice3Img, 4.25, {
        skewX: 6,
        rotation: 1.5,
        x: -33,
        scaleX: 0.98,
...
function () {
  ...
  function Header() {
    ...
    // Elements Varibles
    this.$Header = $("header");
    this.$ToggleNavBtn = $(".toggle-btn");
    this.$SidebarNav = $(".global-nav");
    this.$SidebarItem = this.$SidebarNav.find(".nav-item");
    this.$CloseNavMenu = $(".close-nav-menu");
    this.$decorElement = this.$SidebarNav.find(".decor-img");
    this.bindEvents();
  }
  /* ===================================
   *  EVENTS
  ...
  _createClass(Header, [{
    ...
    key: "HeaderSetup",
    value: function HeaderSetup() {
      var _this = this;

      this.blockMenuInteraction = false;
      this.DecorAnimation();
      this.BuildMenuAnimation();
      this.$ToggleNavBtn.on("click", function (e) {
        if (!_this.blockMenuInteraction) {
          _this.blockMenuInteraction = true; // Toggle Menu

          if (!_this.$ToggleNavBtn.hasClass("active")) {
            _this.ShowMenu();
          } else {
            _this.HideMenu();
          }
      ...
      this.$CloseNavMenu.click(function (e) {
        if (!_this.blockMenuInteraction && _this.$ToggleNavBtn.hasClass("active")) {
          // Turn The Flag On
          _this.blockMenuInteraction = true;
    ...
    value: function ShowMenu() {
      ...
      this.$ToggleNavBtn.addClass("active");
      this.ShowMenuAnimation.progress(0);
      this.ShowMenuAnimation.timeScale(1).play(); // Release the flag after 400ms

      setTimeout(function () {
        _this2.blockMenuInteraction = false;
      }, 750);
    }
  }, {
    key: "HideMenu",
    value: function HideMenu() {
      ...
      this.$ToggleNavBtn.removeClass("active"); // this.HideMenuAnimation.progress(0);

      this.ShowMenuAnimation.timeScale(2).reverse(); // Release the flag after 400ms

      setTimeout(function () {
        _this3.blockMenuInteraction = false;
      }, 370);
    }
  }, {
    key: "BuildMenuAnimation",
    value: function BuildMenuAnimation() {
      TweenMax.set(this.$SidebarItem, {
        autoAlpha: 0
      }); // Show Menu Animation Build

      this.ShowMenuAnimation = new TimelineMax({
        paused: true // onComplete: () => this.decorAnimation.play(),

      });
      this.ShowMenuAnimation.add("start");
      this.ShowMenuAnimation.fromTo(this.$SidebarNav, 0.5, {
        y: '-100%'
      }, {
        y: '0%'
      });
      ...
      if (!IS_MOBILE) {
        this.ShowMenuAnimation.fromTo(this.$SidebarItem, 0.25, {
          x: -35,
          autoAlpha: 0,
        ...
        this.ShowMenuAnimation.fromTo(this.$SidebarItem, 0.35, {
          y: 35,
          autoAlpha: 0,
          ease: _gsap.Power2.easeOut
        }, {
    ...
    key: "DecorAnimation",
    value: function DecorAnimation() {// this.decorAnimation = new TimelineMax({
      //   repeat: -1,
      //   yoyo: true,
      //   repeatDelay: 0.35,
      //   paused: true,
      // });
      // this.decorAnimation.add("start");
      // this.decorAnimation.to(this.$decorElement,4,{
      //   skewX: 7,
      //   rotation: 2,
      //   x: -30,
      //   scaleX: 0.98,
...
function () {
  ...
  function Common() {
    ...
    // Elements Variables
    this.$BannerSection = $('#home-banner');
    this.$BannerSlider = this.$BannerSection.find('#sapporo-home-banner-slider'); // Elements
...
function () {
  ...
  function Common() {
    ...
    // Elements Variables
    this.$GallerySection = $("#home-gallery");
    this.$TopicSlider = this.$GallerySection.find("#gallery-topic-slider"); // Background Layer

    this.$DecorLayer = this.$GallerySection.find(".decor-layer");
    this.$BeerFlowerIMG = this.$DecorLayer.find(".beer-flower-img");
    this.$WheatIMG = this.$DecorLayer.find(".wheat-img");
    this.GalleryListener = new _utils.pageListener(); // Elements
  ...
  _createClass(Common, [{
    key: "bindEvents",
    value: function bindEvents() {
      // Decoration Animation Setup
      this.DecorAnimationSetup(); // Smooth Scrolling Setup

      this.GallerySliderSetup();
    }
    /* ===================================
     *  METHODS
     * =================================== */
    ...
    key: "GallerySliderSetup",
    value: function GallerySliderSetup() {
      this.$TopicSlider.slick({
        slidesToShow: 4,
        nextArrow: "<div class=\"topic-slide next-slide\"><i class=\"fas fa-angle-right\"></i></div>",
        prevArrow: "<div class=\"topic-slide prev-slide\"><i class=\"fas fa-angle-left\"></i></div>",
        responsive: [{
          breakpoint: 769,
          settings: {
            slidesToShow: 1
      ...
      this.SliderCarouselPairSetup();
      $(document).on("click", ".carousel-item", function (e) {
        var $target = $(e.target);

        if ($target.parents(".carousel-item").length > 0) {
          $target = $target.parents(".carousel-item");
        }

        var carouselTargetID = $target.parents(".gallery-carousel").attr("id"); // let sliderTargetID = carouselTargetID.replace('gallery-carousel','gallery-slider');

        var $nextIndex = $target.data("slide-to"); // console.log(carouselTargetID,$nextIndex,sliderTargetID);

        $("#".concat(carouselTargetID)).slick("slickGoTo", $nextIndex);
      });
    }
  }, {
    key: "SliderCarouselPairSetup",
    value: function SliderCarouselPairSetup() {
      var _this = this;

      this.$SlidersBlock = this.$GallerySection.find('.sliders-block-holder');
      var totalSlide = this.$SlidersBlock.length;
      console.log(totalSlide);

      for (var i = 1; i <= totalSlide; i++) {
        $("#gallery-slider-".concat(i)).slick({
          fade: true,
          arrows: false,
          asNavFor: "#gallery-carousel-".concat(i),
          infinite: false
        });
        $("#gallery-carousel-".concat(i)).slick({
          asNavFor: "#gallery-slider-".concat(i),
          slidesToShow: 4,
          nextArrow: "<div class=\"carousel-slide next-slide\"><span class=\"caret\"></span></div>",
          prevArrow: "<div class=\"carousel-slide prev-slide\"><span class=\"caret\"></span></div>",
          infinite: false
        });
      } // Swicth Gallery Setup


      $(document).on('click', '.sliders-tab-selector .tab-item', function (e) {
        var $target = $(e.target);

        if ($target.parents('.tab-item').length > 0) {
          $target = $target.parents('.tab-item');
        }

        if (!$target.hasClass('active')) {
          var targetSlide = $target.data('ref-slider');

          _this.GalleryListener.emit('change-album', targetSlide);
        }
      });

      if (IS_MOBILE) {
        $("#gallery-topic-slider").on('afterChange', function (slick, event, currentSlide) {
          _this.GalleryListener.emit('change-album', currentSlide + 1);
        });
      }

      this.GalleryListener.on('change-album', function (slideID) {
        // Album Name Change
        $('.sliders-tab-selector .tab-item.active').removeClass('active');
        $(".sliders-tab-selector .tab-item[data-ref-slider=".concat(slideID, "]")).addClass('active'); // Slide Container Change

        $('.sliders-block-holder.active').removeClass('active');
        $("#slider-block-no-".concat(slideID)).addClass('active');
      });
    }
  }, {
    key: "DecorAnimationSetup",
    value: function DecorAnimationSetup() {
      // Wheat Animation
      this.wheatAnimation = new TimelineMax({
        repeat: -1,
        yoyo: true,
        repeatDelay: 0.35
      });
      this.wheatAnimation.add("start");
      this.wheatAnimation.to([this.$WheatIMG], 4, {
        skewX: 5,
        rotation: 1,
        x: -10,
        scaleX: 0.99,
        transformOrigin: "0% 100%"
      }); // Beer Flower Animation

      this.BeerFlowerAnimation = new TimelineMax({
        repeat: -1,
        repeatDelay: 0.5
      });
      this.BeerFlowerAnimation.add("start");
      this.BeerFlowerAnimation.to(this.$BeerFlowerIMG, 3, {
        rotation: 3,
        x: -7,
        transformOrigin: "100% 0%",
        ease: _gsap.Power1.easeOut
      });
      this.BeerFlowerAnimation.to(this.$BeerFlowerIMG, 2, {
        rotation: 2,
        x: -5,
        transformOrigin: "100% 0%",
        ease: _gsap.Linear.easeNone
...
function () {
  ...
  function Common() {
    ...
    // Elements Variables
    this.$SaleSection = $("#home-sale-info"); // Decoration Elements

    this.$WheatIMG = this.$SaleSection.find(".rice-flower");
    this.$StarIMG = this.$SaleSection.find(".star-image"); // Main Content Elements

    this.$MainContent = this.$SaleSection.find(".main-content");
    this.$SaleContextList = this.$MainContent.find(".sale-context-list");
    this.$SaleContextItem = this.$SaleContextList.find(".sale-context-item");
    this.$SaleSlider = this.$MainContent.find(".sale-image-slider");
    this.$SaleSliderItem = this.$SaleSlider.find(".sale-image-slider-item");
    this.$SliderPaging = this.$MainContent.find(".slide-paging");
    this.$PagingItem = this.$SliderPaging.find(".sale-slide-control");
    this.$SaleDetailModal = this.$SaleSection.find(".sale-detail-modal-layer");
    this.$OpenSaleDetailModal = this.$SaleSection.find(".see-more-content");
    this.$CloseSaleDetailModal = this.$SaleSection.find(".close-sale-detail-modal");
    this.SliderListener = new _utils.pageListener(); // Elements
  ...
  _createClass(Common, [{
    key: "bindEvents",
    value: function bindEvents() {
      // Setup Decor Animation
      this.SetupDecorAnimation(); // Setup Sale Slider

      this.SetupSaleSlider();
    }
    /* ===================================
     *  METHODS
     * =================================== */
    ...
    key: "SetupDecorAnimation",
    value: function SetupDecorAnimation() {}
  }, {
    key: "SetupSaleSlider",
    value: function SetupSaleSlider() {
      var _this = this;

      this.InitState();
      this.DecorAnimation();
      this.SliderEventsSetup(); // Bind Global Event

      this.SliderListener.on("slide-to", function (index) {
        _this.UpdateSlider(index);
      });
    }
  }, {
    key: "InitState",
    value: function InitState() {
      // Check the HTML and build Slider State object
      var totalSlide = this.$SaleSliderItem.length; // Build The State

      this.sliderState = {
        oldSlide: null,
        readySlide: 2,
        prevSlide: totalSlide,
        // Final Slide Is Previous
    ...
    value: function UpdateSlider(index) {
      ...
      if (!this.sliderState.BlockSlider) {
        this.sliderState.BlockSlider = true; // Update State

        var newSlide = index;
        var currentSlide = this.sliderState.currentSlide;
        var readySlide = index === this.$SaleContextItem.length ? 1 : index + 1;
        console.log(this.sliderState); // Check if need update

        if (currentSlide !== newSlide) {
          // update new state
          this.sliderState.oldSlide = currentSlide;
          this.sliderState.currentSlide = newSlide;
          this.sliderState.readySlide = readySlide;
          /* Update Image & Prepare Next Slide */
          // Clean old state Slide

          $(".sale-image-slider-item.active").removeClass("active").addClass("removing");
          $(".sale-image-slider-item.ready").removeClass("ready"); // Build New Slide

          $("#slide-sale-no-".concat(this.sliderState.currentSlide)).addClass("active");
          $("#slide-sale-no-".concat(this.sliderState.readySlide)).addClass("ready");
          /* Update Paging */
          // Clean old state Paging

          $(".sale-slide-control.active").removeClass("active"); // Active New Paging

          $(".sale-slide-control[data-target-slide=".concat(this.sliderState.currentSlide, "]")).addClass("active");
          /* Update Info */
          // Clean old stage info

          $(".sale-context-item.active").removeClass("active"); // Active New Info

          $(".sale-context-item[data-sale-item=".concat(this.sliderState.currentSlide, "]")).addClass("active"); // Update Modal

          $(".sale-detail-item.active").removeClass("active");
          $("#sale-detail-modal-context-".concat(this.sliderState.currentSlide)).addClass("active"); // Cleanup

          setTimeout(function () {
            // Release The Flag
            _this2.sliderState.BlockSlider = false; // Release removing Class

            $(".sale-image-slider-item.removing").removeClass("removing");
          }, 400);
        }
      }
    }
  }, {
    key: "SliderEventsSetup",
    value: function SliderEventsSetup() {
      var _this3 = this;

      // Click on the ready Slide
      $(document).on("click", ".sale-image-slider-item.ready", function (e) {
        var $target = $(e.target);

        if ($target.parents(".sale-image-slider-item.ready").length > 0) {
          $target = $target.parents(".sale-image-slider-item.ready");
        }

        var targetSlideIndex = $target.data("target-slide");
        console.log(targetSlideIndex);

        _this3.SliderListener.emit("slide-to", targetSlideIndex);
      }); // Click On Paging Button

      this.$PagingItem.on("click", function (e) {
        var $target = $(e.target);
      ...
      }); // Click On Open Sale Detail Modal

      this.$OpenSaleDetailModal.on("click", function (e) {
        e.preventDefault();

        _this3.$SaleDetailModal.addClass("active");
      });
      this.$CloseSaleDetailModal.on("click", function () {
        _this3.$SaleDetailModal.removeClass("active");
      });
      jquerySwipeHandler.handleSwipe('#sale-image-slider', [jquerySwipeHandler.SWIPE_LEFT, jquerySwipeHandler.SWIPE_RIGHT, jquerySwipeHandler.SWIPE_DOWN, jquerySwipeHandler.SWIPE_UP, jquerySwipeHandler.CLICK], function (direction) {
        var targetSlideIndex = 1;

        switch (direction) {
          // Next Slide
          case 'SWIPE_LEFT':
            targetSlideIndex = _this3.sliderState.currentSlide === _this3.$SaleContextItem.length ? 1 : _this3.sliderState.currentSlide + 1;
          ...
          case 'SWIPE_RIGHT':
            targetSlideIndex = _this3.sliderState.currentSlide === 1 ? _this3.$SaleContextItem.length : _this3.sliderState.currentSlide - 1;
    ...
    value: function DecorAnimation() {
      ...
      this.decorAnimation.add("start");
      this.decorAnimation.to(this.$WheatIMG, 4, {
        skewX: 5,
        rotation: 1,
        x: -10,
        scaleX: 0.99,
      ...
      }); // CreateInfiniteAnimation(this.$StarIMG);
    }
  }]);
